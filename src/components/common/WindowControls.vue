<template>
  <div class="flex items-center space-x-1">
    ...
    <button
      ...
      @mousedown.stop
    >
      <Squares2X2Icon v-if="!isMaximized" class="w-4 h-4 text-gray-600 dark:text-gray-300" />
      <Squares2X2Icon v-else class="w-4 h-4 text-gray-600 dark:text-gray-300 rotate-45" />
    </button>

    <button
      class="w-7 h-7 flex items-center justify-center rounded hover:bg-red-500 hover:text-white"
...
onMounted(async () => {
  ...
  try {
    ...
    if (typeof windowApi.onMaximize === 'function') {
      listeners.push(await windowApi.onMaximize(() => (isMaximized.value = true)))
    }
    if (typeof windowApi.onUnmaximize === 'function') {
      listeners.push(await windowApi.onUnmaximize(() => (isMaximized.value = false)))
    }
  } catch (error) {
