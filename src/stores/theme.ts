...
import { ToolboxAPI, type ThemeMode, isTauriAvailable } from '@/api'
...
export const useThemeStore = defineStore('theme', () => {
  ...
  const fetchInitialTheme = async () => {
    ...
    try {
      const value = await ToolboxAPI.readTheme()
  ...
  const setTheme = async (next: ThemeMode) => {
    ...
    if (isTauriAvailable()) {
      try {
        await window.__TAURI__.invoke('save_config', { key: 'theme', value: next })
      } catch (error) {
