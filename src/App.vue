<template>
  <div ref="appShellRef" class="app-shell">
    ...
    <main ref="mainRef" class="relative flex justify-center px-6 py-6">
      ...
      <section ref="cardRef" class="card-apple mx-auto w-full max-w-3xl space-y-8">
...
const parsePxValue = (value: string) => Number.parseFloat(value) || 0
...
const synchronizeWindowSize = () => {
  ...
  resizeAnimationFrame = requestAnimationFrame(async () => {
    ...
    const horizontalPadding =
      mainStyles ? parsePxValue(mainStyles.paddingLeft) + parsePxValue(mainStyles.paddingRight) : 0
    const verticalPadding =
      mainStyles ? parsePxValue(mainStyles.paddingTop) + parsePxValue(mainStyles.paddingBottom) : 0

    const titleBarElement = shellElement.querySelector<HTMLElement>('.titlebar')
    const titleBarHeight = titleBarElement ? titleBarElement.getBoundingClientRect().height : 0

    let width = Math.ceil(shellRect.width)
    let height = Math.ceil(shellRect.height)

    if (cardRect) {
      const verticalBuffer = 24
      width = Math.ceil(cardRect.width + horizontalPadding)
      height = Math.ceil(cardRect.height + verticalPadding + titleBarHeight + verticalBuffer)
    }
